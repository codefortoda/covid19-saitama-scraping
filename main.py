...
def export_data_json():
    ...
    for i in re.finditer(
        r"(陽性確認者数|新規公表分|指定医療機関|一般医療機関|最重症者|重症者|宿泊療養|自宅療養|宿泊療養予定\(宿泊療養施設への入室予定として調整している者\)|入院予定\(入院予定として調整する可能性がある者\)|新型コロナウイルス感染症を死因とする死亡|死亡|新規公表分|退院・療養終了)：?([0-9,]+)人?",
        text,
    ):
    ...
    temp["自宅療養等"] = temp["自宅療養"] + temp["宿泊療養予定(宿泊療養施設への入室予定として調整している者)"] + temp["入院予定(入院予定として調整する可能性がある者)"]
    ...
    # 状況
    jokyo_path = fetch_csv(settings.JOKYO_URL, settings.JOKYO_TITLE)
